...
export function FilmstripPanel({ slideFrames, isOpen, onClose }: FilmstripPanelProps) {
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 bg-white rounded-xl shadow-xl border border-slate-200 flex flex-col"
      style={{ width: 480 }}>
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => console.warn('export stub — Phase 2')}
        ...
        {slideFrames.length === 0 ? (
          ...
          slideFrames.map((frame, i) => (
            ...
            >
              ...
              <span
                draggable
                className="cursor-grab text-slate-300 text-xs select-none"
                title="Drag to reorder (Phase 2)"
              >
                ⠿
              </span>
              {/* Thumbnail placeholder */}
              <div className="w-20 h-14 rounded bg-slate-100 border border-slate-200 flex items-center justify-center">
                <span className="text-slate-400 text-xs font-bold">{(frame.slide_index ?? i) + 1}</span>
              </div>
