...
function MemberList({
  ...
  return (
    <div className="mt-2">
      <p className="mb-1 text-xs font-medium text-indigo-600">Collaborators</p>
      {filtered.length === 0 ? (
      ...
      <ul className="space-y-1">
        {toShow.map((m) => {
          ...
          return (
            <li key={m.user_id} className="flex items-center gap-2 text-xs text-slate-600">
              <span className="min-w-0 flex-1 truncate">{label}</span>
              <span className="w-14 shrink-0 text-right text-slate-500">{statusLabel}</span>
...
export function BoardCard({
  ...
  return (
    <div
      ...
      className="group flex min-h-[280px] cursor-pointer flex-col rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-indigo-200 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
    >
      ...
      <div className="mt-auto flex items-center justify-between pt-2">
        ...
        {isOwner ? (
          ...
          <button
            ...
            className="rounded border border-red-500 px-2 py-1 text-xs font-medium text-red-500 transition hover:bg-red-500 hover:text-white"
            title="Leave board"
          >
            Leave
          </button>
